...
it('clients binder is available on the client', () => {
...
describe('getClient', () => {
  ...
  test('with embedded organization', () => {
    return new NetworkMocker(getApiKeyClientProvider()).use(async ([mollieClient, networkMocker]) => {
      networkMocker
        .intercept('GET', '/clients/org_12345678', 200, {
          resource: 'client',
  ...
  test('helper fetches organization if not embedded', () => {
    return new NetworkMocker(getApiKeyClientProvider()).use(async ([mollieClient, networkMocker]) => {
  ...
  test('with embedded onboarding', () => {
    return new NetworkMocker(getApiKeyClientProvider()).use(async ([mollieClient, networkMocker]) => {
        ...
        .intercept('GET', '/clients/org_12345678', 200, {
          ...
          _embedded: {
            onboarding: {
              resource: 'onboarding',
              name: 'Test Company',
              signedUpAt: '2023-04-06T13:10:19+00:00',
              status: 'completed',
              ...
              _links: {
                ...
                documentation: {
                  href: 'https://docs.mollie.com/reference/onboarding-api',
                  type: 'text/html',
                },
              },
            },
      ...
      // Onboarding should be embedded
      expect(client._embedded?.onboarding).toBeDefined();
      expect(client._embedded?.onboarding?.name).toBe('Test Company');

      // getOnboarding() should return the embedded onboarding without extra API call
      const onboarding = await client.getOnboarding();
      expect(onboarding.name).toBe('Test Company');
      expect(onboarding.status).toBe('completed');
    });
  });

  test('with embedded capabilities', () => {
    return new NetworkMocker(getApiKeyClientProvider()).use(async ([mollieClient, networkMocker]) => {
      networkMocker
        .intercept('GET', '/clients/org_12345678', 200, {
          resource: 'client',
          ...
          _embedded: {
            capabilities: [
              ...
              },
              {
                ...
                _links: {
                  self: {
                    href: 'https://api.mollie.com/v2/capabilities/refunds',
                    type: 'application/hal+json',
                  },
                },
              },
      ...
      // Capabilities should be embedded
      expect(client._embedded?.capabilities).toBeDefined();
      expect(client._embedded?.capabilities).toHaveLength(2);

      // getCapabilities() should return the embedded capabilities
      const capabilities = client.getCapabilities();
      expect(capabilities).toHaveLength(2);
      expect(capabilities[0].name).toBe('payments');
      expect(capabilities[1].name).toBe('refunds');
    });
  });
});
...
test('listClients', () => {
  return new NetworkMocker(getApiKeyClientProvider()).use(async ([mollieClient, networkMocker]) => {
    ...
    expect(clients[1].commission).toBeUndefined();
